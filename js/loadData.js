...
function loadData() {
  ...
  const svg = d3.select("#canvas_histFCE")
                .append("svg")
  ...
  d3.tsv('data/tsv/'+pfx+'/SummaryAnalyzeEfficiency_'+runNumber+'_Express2021.tsv').then(function(effData) {
    ...
    var LegndSvg = d3.select("#legend");
    for (i=0; i<colorList.length; i++) {
      LegndSvg.append("rect").attr("width", "40px").attr("height", "16px")
         .attr("x", String(70*i + 360) + "px").attr("y", "10px").style("fill", colorList[i]).style("stroke", "#777");
      LegndSvg.append("text").attr("x", String(70*i + 380)+"px").attr("y", "18px").text(labelList[i])
    ...
    .value(function(d) { return d.Fiducial_Cut_Efficiency; })   // I need to give the vector of value
